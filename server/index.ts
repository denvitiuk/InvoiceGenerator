...
// Продакшн-выдача фронта (если собран web/dist)
const WEB_DIST = path.join(ROOT, "web", "dist");
if (fs.existsSync(WEB_DIST)) {
    app.use(express.static(WEB_DIST));
    // SPA-фоллбек: не перехватывать API/статику
    app.get(/^(?!\/(preview|render|render-all|upload|presets|download|templates|assets|i18n|health)\b).*/, (_req: any, res: any) => {
...
registerDownload(app);     // GET /download -> отдача файлов из /out
